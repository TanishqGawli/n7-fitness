...
export default function Gallery() {
  ...
  // 🔥 Softer premium parallax (less extreme = more luxury)
  ...
  return (
    ...
    >
      ...
      >
        <p className="tracking-[0.4em] text-red-500">GALLERY</p>
      ...
      <div className="relative mt-32 grid grid-cols-1 gap-10 md:grid-cols-3">
        ...
        <motion.div style={{ y: y1 }} className="space-y-10">
          {images.slice(0, 4).map((img, i) => (
            <GlassImage key={i} img={img} height="h-[420px]" />
          ))}
        </motion.div>
        ...
        <motion.div style={{ y: y2 }} className="space-y-10">
          {images.slice(4, 8).map((img, i) => (
            <GlassImage key={i} img={img} height="h-[560px]" />
          ))}
        </motion.div>
        ...
        <motion.div style={{ y: y3 }} className="space-y-10">
          {images.slice(8, 12).map((img, i) => (
            <GlassImage key={i} img={img} height="h-[480px]" />
          ))}
        </motion.div>
      </div>
      ...
      >
        <h3 className="text-4xl font-black md:text-6xl">
          ...
          <br />
          <span className="text-red-600">IT’S A MOVEMENT.</span>
        </h3>
      </motion.div>
    </section>
  );
...
function GlassImage({ img, height }) {
  return (
    <motion.div
    ...
    >
      ...
      {/* LUXURY OVERLAY */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-black/10 opacity-60 group-hover:opacity-30 transition" />

      {/* SOFT BORDER GLOW */}
      <div className="absolute inset-0 rounded-[32px] ring-1 ring-white/10 group-hover:ring-red-500/30 transition" />
    </motion.div>
  );
}